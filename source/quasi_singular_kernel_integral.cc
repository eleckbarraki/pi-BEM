...
template <int dim>
Point<dim-1>
QuasiSingularKernelIntegral<dim>::find_closest_reference_cell_point(const Point<dim> &external_point)
{
  Point<dim-1> eta_sol;
  ...
  else
  {
    ...
    {
      // here's the quadrature rule obtained with the solution point as the only point
      ...
      const std::vector<Point<3>> &sol_q_points_spacedim = sol_fe_values.get_quadrature_points();
      ...
      // std::cout<<"Iteration: "<<iter_counter<<"  Residual: "<<residual.l2_norm()<<std::endl;
      iter_counter++;
    } // end while      
    ...
    {
      double minimum_distance_among_faces = std::numeric_limits<double>::infinity();
      unsigned int face_with_minimum_distance=GeometryInfo<2>::faces_per_cell;
      // in this case we must look for the closest point, but on the boundary
      // we'll use bisection in the interval [0,1]
      for (unsigned int f = 0; f < GeometryInfo<2>::faces_per_cell; ++f)
      {
        ...
        while (fabs(residual_line) > 1e-7 && iter_counter_line < 30)
        {
          ...
          FEFaceValues<2, 3> sol_fe_values_line(mapping,
                                                fe,
                                                sol_quadrature_line,
          ...
          auto sol_jacobian_grad_line      = sol_fe_values_line.jacobian_grad(0);
          
          // this is the distance vector between the external point and the
          // current point on the cell
          Tensor<1,3> dist_vect_line;
          dist_vect_line[0] = external_point[0]-sol_q_points_spacedim_line[0][0];
          dist_vect_line[1] = external_point[1]-sol_q_points_spacedim_line[0][1];
          dist_vect_line[2] = external_point[2]-sol_q_points_spacedim_line[0][2];
          min_dist_line = dist_vect_line.norm();
          // std::cout<<"Min Dist Line "<<f<<": "<<min_dist_line<<std::endl;
          ...
          double jacobian_line = 2.0*dq_du_line*dq_du_line-2.0*ddq_duu*dist_vect_line;
          ...
          //std::cout<<"Face "<<f<<"  Iteration: "<<iter_counter_line<<"  Residual: "<<residual_line<<" Distance: "<<min_dist_line<<"  Solution: "<<solution_line<<" 3D Point: "<<sol_q_points_spacedim_line[0]<<std::endl;
        ...
        {
          //std::cout<<"Solved"<<std::endl;
          //std::cout<<"Min Dist: "<<min_dist_line<<"  Solution line: "<<solution_line<<std::endl;
          if (solution_line > 0 && solution_line < 1 )
            if (min_dist_line<minimum_distance_among_faces)
            {
              ...
              switch (f)
              {
                ...
                break;
                default:
                AssertThrow(true, ExcMessage("Cell has four faces and we are not in face 0,1,2,3"));
                break;
              }
            } // end if
        } // end else
      ...
      if (minimum_distance_among_faces == std::numeric_limits<double>::infinity())
      {
        double minimum_distance_among_vertices = minimum_distance_among_faces;
        ...
        // std::cout<<"Minimum distance found among vertices: "<<minimum_distance_among_vertices<<std::endl;
      }
    } // end if    
  } // end else (dim 3)
  ...
  Point<3> real_min_distance_point = mapping.transform_unit_to_real_cell(cell, eta_sol);
  this->min_distance = external_point.distance(real_min_distance_point);
  // std::cout<<"Real cell min distance point coordinates: "<<real_min_distance_point<<std::endl;
  // std::cout<<"Min distance: "<<min_distance<<std::endl;
  // std::cout<<"Cell size: "<<cell->diameter()<<std::endl;
  
  return eta_sol;
}


//template class QuasiSingularKernelIntegral<2>;
//template class QuasiSingularKernelIntegral<3>;
