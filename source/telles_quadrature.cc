...
//  // find closest point on element (clamped)
//  Point<2> ref_closest;
//  try 
//  {
//    ref_closest = mapping.transform_real_to_unit_cell(cell, singularity);
//    // clamp to [0,1]^2
//    for (unsigned int d = 0; d < 2; ++d)
//      ref_closest[d] = std::max(0.0, std::min(1.0, ref_closest[d]));
//  }
//  catch (...) 
//  {
//    ref_closest = Point<2>(0.5, 0.5);
//  }
...
//  weights.reserve(n*n);
  ...
  std::cout << "ref point " << ref_qsing << "    D1, D2: " << D1 << "  " << D2 << std::endl;
